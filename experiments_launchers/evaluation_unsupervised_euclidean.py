...
for i in tqdm.trange(args.n):
    total_accuracy, std = evaluation.unsupervised_euclidean_eval(representations, D.Y, n_centroid,  verbose=False)
    results.append(total_accuracy)
    std_kmeans.append(std)

R = torch.Tensor(results)
S = torch.Tensor(std_kmeans)
log_xp.append({"evaluation_unsupervised_euclidean": {"RES":R.tolist(), "MIN":R.min().item(),
